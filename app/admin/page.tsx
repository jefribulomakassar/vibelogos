...
function LogoGroundImporter({ adminToken, onImport, defaultUrl = '' }: LogoGroundImporterProps) {
  ...
  const handleScrape = async () => {
    ...
    setLoading(true); setError(''); setPreview(null); setImported(false);
    try {
  ...
  return (
    <div style={{
      ...
      border: '1px solid rgba(200,245,66,0.25)', borderRadius: 14, padding: 20,
      display: 'flex', flexDirection: 'column', gap: 14,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(200,245,66,0.15)', border: '1px solid rgba(200,245,66,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>⚡</div>
        <div>
          <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 14, color: 'var(--accent)' }}>Import dari LogoGround</div>
          <div style={{ fontSize: 11, color: '#666', marginTop: 1 }}>Auto-isi title, deskripsi, keywords, kategori, harga & gambar logo</div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <input
          ...
          style={{ ...inputStyle, flex: 1, fontSize: 13, borderColor: error ? 'rgba(255,68,68,0.5)' : 'rgba(200,245,66,0.2)' }}
          disabled={isLoading}
        />
        <button
          ...
          style={{
            ...
            border: '1px solid rgba(200,245,66,0.2)', transition: 'all 0.2s',
          }}
        >
          {loading ? (<><span style={{ display: 'inline-block', width: 13, height: 13, border: '2px solid rgba(200,245,66,0.3)', borderTopColor: 'var(--accent)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />Mengambil…</>) : '🔍 Ambil Data'}
        </button>
      </div>

      {error && (
        <div style={{ background: 'rgba(255,68,68,0.08)', border: '1px solid rgba(255,68,68,0.25)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#ff6b6b', display: 'flex', gap: 6 }}>⚠️ {error}</div>
      )}
      {imported && !error && (
        <div style={{ background: 'rgba(200,245,66,0.1)', border: '1px solid rgba(200,245,66,0.3)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: 'var(--accent)', display: 'flex', gap: 6, alignItems: 'center' }}>✓ Data berhasil diimport ke form!</div>
      )}
          ...
          <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
            {preview.logo_url && (
              <div style={{ width: 72, height: 72, borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg)', flexShrink: 0, position: 'relative', overflow: 'hidden' }}>
                ...
                <img src={preview.logo_url} alt={preview.title} style={{ width: '100%', height: '100%', objectFit: 'contain', padding: 8 }} crossOrigin="anonymous" />
              </div>
            )}
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 15, marginBottom: 4 }}>{preview.title}</div>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {preview.price > 0 && <span style={{ fontSize: 12, color: 'var(--accent)', fontWeight: 600 }}>${preview.price}</span>}
                {preview.main_category && <span className="tag">{preview.main_category}</span>}
                {preview.secondary_categories.map(c => <span key={c} className="tag" style={{ opacity: 0.6 }}>{c}</span>)}
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {preview.description && (
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1.5, maxHeight: 60, overflow: 'hidden' }}>{preview.description}</div>
              </div>
            )}
            {preview.keywords.length > 0 && (
              <div>
                ...
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  ...
                  {preview.keywords.length > 12 && <span style={{ fontSize: 11, color: '#555' }}>+{preview.keywords.length - 12} lagi</span>}
                </div>
              </div>
            )}
          </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', background: 'var(--bg3)', borderRadius: 6, padding: 2, gap: 2 }}>
          {(['upload', 'url'] as const).map(m => (
            <button key={m} onClick={() => setMode(m)} style={{ padding: '3px 10px', borderRadius: 5, border: 'none', fontSize: 11, fontFamily: 'Syne, sans-serif', fontWeight: 600, background: mode === m ? 'rgba(200,245,66,0.15)' : 'transparent', color: mode === m ? 'var(--accent)' : '#555', cursor: 'pointer', transition: 'all 0.15s' }}>
      ...
      {mode === 'url' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            >
              {loadingUrl ? <span style={{ display: 'inline-block', width: 12, height: 12, border: '2px solid rgba(200,245,66,0.3)', borderTopColor: 'var(--accent)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /> : '⬇️'}
...
function AIMockupGenerator({ logoUrl, title, category, adminToken, mockups, onMockupsChange }: {
  ...
  const removeResult = (i: number) => {
    ...
    setResults(next); onMockupsChange(next.map(r => r.url));
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {results.length > 0 && <span style={{ fontSize: 11, color: 'var(--muted)' }}>{results.length} gambar</span>}
      </div>

      <div style={{ display: 'flex', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 10, padding: 4, gap: 4 }}>
      ...
      {activeTab === 'ai' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          >
            {generating ? (
              <><span style={{ display: 'inline-block', width: 16, height: 16, border: '2px solid var(--muted)', borderTopColor: 'var(--accent)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />{STEP_MESSAGES[stepIdx]}</>
            ) : (
              <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg>{results.length > 0 ? '🔄 Regenerate Mockups' : '✨ Generate AI Mockups'}</>
            )}
          </button>
          {!logoUrl && <p style={{ fontSize: 12, color: '#555', textAlign: 'center' }}>Upload logo terlebih dahulu untuk generate mockup</p>}
          {error && <div style={{ background: 'rgba(255,68,68,0.1)', border: '1px solid rgba(255,68,68,0.3)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#ff6b6b' }}>⚠️ {error}</div>}
        </div>
      )}
      ...
      {activeTab === 'upload' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div
            ...
            style={{ border: `2px dashed ${dragging ? 'var(--accent)' : 'rgba(200,245,66,0.25)'}`, borderRadius: 12, padding: '28px 20px', textAlign: 'center', cursor: isUploadingManual ? 'default' : 'pointer', transition: 'all 0.2s', background: dragging ? 'rgba(200,245,66,0.04)' : 'transparent' }}
          >
            {isUploadingManual ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ width: '100%', maxWidth: 260, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {Object.entries(uploadProgress).map(([key, pct]) => (
                    <div key={key}>
                      <div style={{ fontSize: 11, color: '#555', marginBottom: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{key.replace(/\d+$/, '')}</div>
          ...
          <input ref={uploadInputRef} type="file" accept="image/*" multiple style={{ display: 'none' }} onChange={e => { if (e.target.files?.length) handleManualFiles(e.target.files); e.target.value = ''; }} />
          <p style={{ fontSize: 11, color: '#555', margin: 0 }}>💡 Hasil upload akan bergabung dengan mockup yang sudah ada di grid bawah.</p>
        </div>
      )}
      ...
      {results.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 12, color: 'var(--muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Semua Mockup ({results.length})</span>
            <button onClick={() => { setResults([]); onMockupsChange([]); }} style={{ fontSize: 11, color: '#ff6b6b', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 6px' }}>Hapus Semua</button>
          </div>
            ...
            {results.map((r, i) => (
              <div key={i} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--bg3)', aspectRatio: '1', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: '6px 8px', background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)' }}>
                  <input type="text" value={r.label} onChange={e => updateLabel(i, e.target.value)} style={{ background: 'transparent', border: 'none', outline: 'none', fontSize: 11, fontWeight: 600, fontFamily: 'Syne, sans-serif', color: '#fff', width: '100%', padding: 0 }} />
                </div>
...
export default function AdminPage() {
  ...
  // ── FIX: tokenRef selalu up-to-date ──────────────────────────────────────
  ...
  // ── fetchLogos: try/catch + cache no-store ────────────────────────────────
  ...
  const handleLogout = () => {
    ...
    setAuthed(false); setToken(''); setLogos([]); setShowForm(false); setDeleteId(null);
  };
  ...
  // ── FIX: handleSave — optimistic update + delay refetch ──────────────────
  const handleSave = async () => {
    ...
    try {
      ...
      setShowForm(false);
      // Background refetch setelah Turso commit
  ...
  // ── FIX: handleDelete — optimistic remove + rollback ─────────────────────
  const handleDelete = async (id: number) => {
    const snapshot = logos;
    ...
    try {
      ...
      if (res.ok) {
        ...
        setLogos(snapshot);
        showToast('⚠️ Hapus gagal, coba lagi.');
      }
    } catch {
      setLogos(snapshot);
      showToast('⚠️ Hapus gagal, cek koneksi.');
    }
  };
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ borderBottom: '1px solid var(--border)', padding: '0 32px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(10,10,10,0.9)', backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button onClick={handleLogout} style={{ padding: '6px 14px', fontSize: 12, background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 7, color: 'var(--muted)', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Logout</button>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(12px)', zIndex: 1000, overflowY: 'auto', display: 'flex', justifyContent: 'center', padding: '32px 16px' }}>
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, maxWidth: 720, width: '100%', height: 'fit-content' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 20 }}>{editId ? '✏️ Edit Logo' : '➕ Add New Logo'}</h2>
            ...
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              {/* FIX: pakai tokenRef.current agar tidak stale */}
              <LogoGroundImporter adminToken={tokenRef.current} onImport={handleLogoGroundImport} defaultUrl={form.logoground_url} />
              ...
              {/* FIX: pakai tokenRef.current */}
              <LogoUploadZone value={form.logo_url} onChange={url => setForm(p => ({ ...p, logo_url: url }))} adminToken={tokenRef.current} />
              ...
              <AIMockupGenerator
                ...
                mockups={form.mockups} onMockupsChange={urls => setForm(p => ({ ...p, mockups: urls }))}
              />
