...
export default function AdminPage() {
  ...
  // ── FIX: simpan token di ref agar selalu up-to-date di dalam async fn ──────
  ...
  // ── fetchLogos: selalu cache: no-store supaya selalu ambil data terbaru ─────
  const fetchLogos = useCallback(async () => {
    ...
    try {
      ...
      // silent — logo tetap tampil dari state sebelumnya
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  // ── FIX: handleSave dengan optimistic update ──────────────────────────────
  const handleSave = async () => {
    ...
    try {
      ...
      // ── Optimistic update: langsung update state tanpa nunggu fetchLogos ──
      if (editId) {
        // Update logo yang diedit di state langsung
        setLogos(prev => prev.map(l =>
          l.id === editId
            ? { ...l, ...payload, id: editId, slug: l.slug, created_at: l.created_at }
            : l
        ));
      } else {
        // Kalau API return logo baru (dengan id & slug), prepend ke list
        if (saved?.logo) {
      ...
      // ── Background refetch setelah delay kecil (beri waktu Turso commit) ──
  ...
  // ── FIX: handleDelete dengan optimistic remove ─────────────────────────────
  const handleDelete = async (id: number) => {
    // Simpan snapshot untuk rollback kalau gagal
    const snapshot = logos;

    // Optimistic: langsung hapus dari UI dulu
    setLogos(prev => prev.filter(l => l.id !== id));
    setDeleteId(null);

    try {
      const res = await fetch(`/api/logos-data/${id}`, {
        method: 'DELETE',
        headers: getHeaders(),
      });
